...
const MilDotCalculator: React.FC = () => {
  ...
  const defaultPhysicalSize = getStorageItem<number>(StorageKeys.MILDOT_PHYSICAL_SIZE_DEFAULT, 0) || 0;
  ...
  const calculateResults = (): void => {
    ...
    try {
      ...
      if (filledFields.includes('mil') && filledFields.includes('size') && !filledFields.includes('distance')) {
        if (isMetric) {
      ...
      else if (filledFields.includes('size') && filledFields.includes('distance') && !filledFields.includes('mil')) {
        if (isMetric) {
      ...
      else if (filledFields.includes('mil') && filledFields.includes('distance') && !filledFields.includes('size')) {
        if (isMetric) {
